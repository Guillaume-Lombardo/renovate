...
description: Use when publishing a Renovate repository snapshot tag to GitHub without pushing the main branch to GitHub.
license: MIT
...
4. Publish the annotated tag with `scripts/publish-github-snapshot.sh vYYYY.MM.DD`.
5. Verify the tag exists on both remotes with `git ls-remote --tags origin <tag>` and `git ls-remote --tags github <tag>`.

Never push `main` to GitHub in the normal workflow.
